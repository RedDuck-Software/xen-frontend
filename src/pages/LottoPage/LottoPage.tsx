...
import {
  ...
  LOTTERYADDRESS,
  XENADDRESS,
} from "../../helpers/constants"; // @todo remove unused var
...
const LottoPage: FC = () => {
  ...
  const [accountBalance, setAccountBalance] = useState<any>(); // @todo remove unused var
  ...
  const [selectedAmountToDeposit, setSelectedAmountToDeposit] = useState<any>(); // @todo remove unused var
  ...
  async function getTotalInfo() {
    ...
    ).toString(); // @todo needless 'await'
    const totalPayout = await (await Lottery.totalPayoutToday()).toString(); // @todo needless 'await'
    const totalAmount = await (
      await Lottery.totalAllTimePrizePool()
    ).toString(); // @todo needless 'await'
    const lastWinner = await (await Lottery.lastWinner()).toString(); // @todo needless 'await'
    const lastWonAmount = await (await Lottery.lastWonAmount()).toString(); // @todo needless 'await'
  ...
  return (
    <div className="wrapper wrapper-lotto">
      ...
      <LottoFooter
        ...
        nextLottoPrize={nextLottoPrize} // @todo fix error
