...
const LottoPage: FC = () => {
    ...
    const [winningPercentage, setWinningPersentage] = useState<string | null>(null);
    ...
    const calculatePercentage = async () => {
        if (!account || !allParticipants.length || !nextLottoPrize) return
        const user = allParticipants.filter((participant: any) => {
            return participant.address === "0x7e1D8F737F65CB0Ae709DB18e6edee652f445903"
        })
        const chanceOfWinning = user[0].tokenAmount / nextLottoPrize.toString();
        return chanceOfWinning
    }


    useEffect(() => {
        if (account) calculatePercentage().then((res) => {
            if (!res) return
            setWinningPersentage(res.toString())
        });
    }, [account, allParticipants, totalPrizePool])
    ...
    const getStaticticsData = async () => {
        ...
        const chancesOfWinning = await contract.totalPrizePool();
        console.log("chancesOfWinning", chancesOfWinning)
